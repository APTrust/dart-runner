...
func main() {
	...
	getLinks := flag.String("get-links", "", "Get public download links for a specific version (e.g. v1.0.4)")
	version := flag.String("version", "", "dart-runner version (e.g. v1.0.4)")
...
func doMakeFolders(client *minio.Client, version string) bool {
	...
	// OS/arch combinations matching the build script output
	arches := []string{
		"linux/amd64",
		"linux/arm64",
		"mac/amd64",
...
func doUpload(client *minio.Client, version, arch, filePath string) bool {
	...
	// arch already contains the os/arch path, e.g. "linux/amd64"
